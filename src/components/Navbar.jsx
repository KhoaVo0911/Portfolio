...
const Navbar = () => {
  return (
    // <nav className="px-6 w-full flex items-center pt-0.5 py-1 fixed top-0 z-40 bg-white/60 backdrop-blur-md font-[mova] relative">
    <nav className="px-6 w-full flex items-center pt-0.5 py-1 min-h-[80px] fixed top-0 z-20 bg-white/75 backdrop-blur-md font-[mova] dark:bg-zinc-900/80 dark:text-white">
              ...
              {navLinks.map((nav) => (
                <NavigationMenuItem key={nav.id} className="px-2 py-0">
                  <NavigationMenuLink
                    ...
                    after:block after:h-[3px] after:bg-black after:rounded-full after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-300 after:origin-center flash-on-hover dark:text-white dark:after:bg-white"
                  >
                    {nav.title}
                  </NavigationMenuLink>
                </NavigationMenuItem>
      ...
      <div className="pb-2 absolute right-0 top-1/2 -translate-y-1/2 pr-4 group-hover:scale-110 transition-transform duration-200">
